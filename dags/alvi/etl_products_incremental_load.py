...
def _incremental_load_products_table(ti):
    ...
    incremental_query = """
        INSERT INTO ecommdata_alvi.productos (id,"""+columns_query+""") 
        VALUES ("""+values_query+""")
        ON CONFLICT (id)
...
with DAG(
    'etl_productos_alvi_incremental_load',
    default_args=default_args,
    description="Extracción y carga de tabla productos desde Janis Alvi Replica hasta Workspace.",
    schedule="0 4 * * *",
    ...
    dag.doc_md = """
    Extracción y carga de tabla de productos de Janis Alvi a Workspace.
