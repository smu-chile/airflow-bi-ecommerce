...
def _incremental_load_pagos_janis(ti):
    ...
    incremental_query = """
        INSERT INTO ecommdata_alvi.pagos_janis (id,"""+columns_query+""") 
        VALUES ("""+values_query+""")
        ON CONFLICT (id)
    ...
    cursor.close()
    pg_connection.close()
    print("Data loaded to Postgres on ecommdata_alvi.pagos_janis")
...
with DAG(
    'etl_pagos_janis_alvi_incremental_load',
    ...
    catchup=False,
    tags=["DATA", "Janis", "ecommdata_alvi", "pagos_janis", "Alvi", "MATIAS"],
) as dag:

    dag.doc_md = """
    Extracción y carga de tabla de pagos de Janis Alvi a Workspace. \n
    ...
    t2 = PythonOperator(
        ...
        op_kwargs = {
            "query": """
                SELECT wo.seq_id
                    , wop.*
                FROM janis_alvicl.wms_order_payments wop
                JOIN janis_alvicl.wms_orders wo
