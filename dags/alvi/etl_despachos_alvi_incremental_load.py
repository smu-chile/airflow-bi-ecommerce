...
def _get_order_shipping_from_janis(ts):
    ...
    query = f"""
        ...
        FROM janis_alvicl.wms_order_shipping as wos 
        LEFT JOIN janis_alvicl.wms_orders wo
...
def _order_shipping_table_incremental_load(ts, ti):
    ...
    incremental_query = """
        INSERT INTO ecommdata_alvi.despachos (id,"""+columns_query+""") 
        VALUES ("""+values_query+""")
        ON CONFLICT (id)
    ...
    cursor.close()
    pg_connection.close()
    print("Data loaded to Postgres. ecommdata_alvi.despachos")
...
with DAG(
    ...
    catchup=False,
    max_active_runs = 1,
    tags=["DATA", "Janis", "ecommdata_alvi", "despachos", "alvi", "cumplimiento_despacho", "MATIAS"],
    on_success_callback=dag_success_slack,
    on_failure_callback=dag_failure_slack,
) as dag:

    dag.doc_md = """
    Extracción y carga de tabla de despachos de Janis Alvi a Workspace. \n
