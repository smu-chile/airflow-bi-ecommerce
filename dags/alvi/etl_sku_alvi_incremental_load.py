...
from utils.janis_alvi_utils import incremental_load_table_s3
...
def _incremental_load_skus_table(ti):
    ...
    incremental_query = """
        INSERT INTO ecommdata_alvi.skus (id,"""+columns_query+""") 
        VALUES ("""+values_query+""")
        ON CONFLICT (id)
...
with DAG(
    'etl_skus_alvi_incremental_load',
    default_args=default_args,
    description="Extracción y carga de tabla skus desde Janis Alvi Replica hasta Workspace.",
    schedule_interval="0 4 * * *",
    start_date=pendulum.datetime(2022, 1, 1, tz="America/Santiago"),
    catchup=False,
    tags=["DATA", "Janis Alvi", "ecommdata_alvi", "skus", "Alvi", "MATIAS"],
    on_success_callback=dag_success_slack,
    on_failure_callback=dag_failure_slack,
) as dag:

    dag.doc_md = """
    Extracción y carga de tabla de skus de Janis Alvi a Workspace. \n
