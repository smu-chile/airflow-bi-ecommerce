...
def _staging_monitor_despacho_table(ti):
    import pandas as pd
    import sqlalchemy
    ...
    # Save to PostgreSQL:
    df.to_sql(name="monitor_despacho_alvi",
                con=engine,         
                schema="staging",         
                if_exists='append',         
                index=False,         
    ...
    print("Data saved to PostgreSQL. Table: staging.monitor_despacho_alvi")
...
with DAG(
    ...
    catchup=False,
    max_active_runs=1,
    tags=["DATA", "janis", "ecommdata_alvi", "monitor_despacho", "alvi"],
) as dag:

    dag.doc_md = """
    Extracción y carga de tabla de monitor_despacho de Janis Alvi a Workspace. \n
    ...
    t4 = PythonOperator(
        task_id = "staging_monitor_despacho_table",
        python_callable = _staging_monitor_despacho_table,
        trigger_rule = "none_failed"
    )

    t5 = PostgresOperator(
        task_id = "monitor_despacho_incremental_load",
        postgres_conn_id="postgresql_conn",
        sql="sql/upsert_monitor_despacho.sql",
    )
