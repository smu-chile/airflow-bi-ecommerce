...
# Canal de Slack de destino (donde se envían las cargas de tienda)
...
def check_vtex_product_status(session, vtex_root_url, vtex_base_url, headers, ref_id):
    """
    Consulta a la API de VTEX Alvi para verificar si un producto/SKU está activo o apagado.
...
# -------------------------------------------------------------------------
# TAREA 1: Extraer productos activos de Janis Alvi con categorías válidas
# -------------------------------------------------------------------------
def _extraer_activos_janis(**kwargs):
    print("Iniciando extracción de SKUs activos de Janis Alvi con categoría comercial válida...")
    ...
    query_janis_activos = """
        ...
        FROM ecommdata_alvi.productos_janis_api j
        JOIN ecommdata_alvi.skus s 
            ON j.ref_id = s.ref_id
        JOIN ecommdata_alvi.productos p 
            ON s.id_producto = p.id
        JOIN ecommdata_alvi.categorias c 
    ...
    print(f"✅ Total SKUs activos en Janis Alvi con categoría válida: {len(lista_ref_ids)}")
    return lista_ref_ids


# -------------------------------------------------------------------------
# TAREA 2: Auditar estado e imágenes en VTEX Alvi en paralelo (20 Workers)
# -------------------------------------------------------------------------
def _auditar_estado_vtex(**kwargs):
    ...
    print(f"Iniciando auditoría en VTEX Alvi para {total_janis_activos} items (20 workers concurrentes)...")

    vtex_account = Variable.get("VTEX_ALVI_ACCOUNT_NAME")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_ref):
            ...
            if completed_count % 500 == 0 or completed_count == total_janis_activos:
                print(f"  Verificados {completed_count}/{total_janis_activos} productos en VTEX Alvi...")
    ...
    print(f"\n==================================================")
    print(f"Total auditados en VTEX Alvi: {total_janis_activos}")
    print(f"Total detectados APAGADOS en VTEX Alvi: {len(items_a_encender)}")
    print(f"==================================================\n")
...
def _notificar_slack(**kwargs):
    ...
    if not items_a_encender:
        msg = f"✅ *[Alvi]* Conciliación Janis vs VTEX finalizada: Todos los *{total_janis_activos}* productos activos en Janis se encuentran correctamente encendidos en VTEX."
    ...
    print(f"📊 Desglose de productos apagados en VTEX Alvi:")
    ...
    create_tbl_sql = """
        CREATE TABLE IF NOT EXISTS ecommdata_alvi.productos_sin_imagen_vtex (
            ref_id TEXT,
            material TEXT,
    ...
    with engine.begin() as conn:
        ...
        conn.execute("TRUNCATE TABLE ecommdata_alvi.productos_sin_imagen_vtex;")

    if len(items_sin_imagen) > 0:
        print(f"Registrando {len(items_sin_imagen)} productos sin imagen en ecommdata_alvi.productos_sin_imagen_vtex...")
    ...
    if len(items_con_imagen) == 0:
        msg = (
            f"ℹ️ *[Alvi - Conciliación VTEX]*\n"
            f"Se detectaron *{len(items_sin_imagen)}* productos apagados en VTEX, pero *ninguno tiene imágenes asociadas*.\n"
            f"⚠️ Se omitió la generación de archivos de encendido y se registraron en `ecommdata_alvi.productos_sin_imagen_vtex`."
        )
        send_text_message(SLACK_CHANNEL_CONCILIACION, msg)
        return
    ...
    file_prod = f"encendido_productos_vtex_alvi_{fecha_str}.csv"
    file_skus = f"encendido_skus_vtex_alvi_{fecha_str}.csv"

    # Construir comentarios para Slack con aviso de omitidos si aplica
    omision_msg = ""
    if len(items_sin_imagen) > 0:
        omision_msg = f"\n⚠️ *Aviso:* Se omitieron *{len(items_sin_imagen)}* productos/SKUs del archivo ya que no tienen imágenes en VTEX (guardados en `ecommdata_alvi.productos_sin_imagen_vtex`)."

    comment_prod = (
        f"📎<!channel> *[Alvi - Encendido Masivo VTEX]*\n"
...
# -------------------------------------------------------------------------
# DEFINICIÓN Y ORQUESTACIÓN DEL DAG
# -------------------------------------------------------------------------
default_args = {
...
with DAG(
    "etl_conciliacion_activos_janis_vtex_alvi",
    default_args=default_args,
    description="Cruce manual entre catálogo activo de Janis Alvi y VTEX con filtro de imágenes",
    schedule_interval=None,  # Ejecución manual únicamente
    start_date=pendulum.datetime(2026, 8, 24, tz="America/Santiago"),
    catchup=False,
    tags=["DATA", "Janis", "VTEX", "Alvi", "conciliacion", "catalogo", "manual", "MAURICIO"],
    on_success_callback=dag_success_slack,
    on_failure_callback=dag_failure_slack,
) as dag:

    dag.doc_md = """
    ## Conciliación Catálogo Activo Janis vs VTEX Alvi (Encendido Masivo con Filtro de Imágenes)

    Pipeline manual modularizado en 3 etapas:
    1. `extraer_activos_janis`: Consulta productos activos en Janis Alvi con categorías válidas desde Postgres (`ecommdata_alvi.productos_janis_api`).
    2. `auditar_estado_vtex`: Audita en tiempo real contra la API de VTEX Alvi (20 workers) detectando apagados y validando imágenes.
    3. `notificar_slack`: 
       - Omite del archivo de encendido aquellos productos que **no tienen imagen en VTEX**.
       - Guarda el registro histórico de los sin imagen en `ecommdata_alvi.productos_sin_imagen_vtex` con timestamp local de Chile.
       - Sube los 2 CSVs con los productos válidos y alerta a Slack (`token_slack_carga_tiendas`).
    """
