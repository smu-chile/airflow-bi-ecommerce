...
def _load_products_store_data(ti):
    ...
    df = pd.read_csv(products_store_object.get()["Body"], dtype={"ref_id": "object"})
    if len(df.index) == 0:
...
with DAG(
    'etl_productos_tienda_alvi_full_load',
    default_args=default_args,
    description="Extracción y carga de tabla productos_tienda desde Janis Alvi Replica hasta Workspace.",
    schedule_interval="0 7 * * *",
    start_date=datetime(2022, 5, 1),
    catchup=False,
    tags=["DATA", "Janis", "ecommdata_alvi", "productos_tienda", "Alvi"],
) as dag:

    dag.doc_md = """
    Extracción y carga de tabla de productos_tienda de Janis Alvi a Workspace. \n
    TRUNCATE - INSERT.
    """ 
    t0 = PythonOperator(
    ...
    t1 = PostgresOperator(
        ...
        sql = "TRUNCATE ecommdata_alvi.productos_tienda"
    )
