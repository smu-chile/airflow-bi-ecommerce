...
def _load_ticket_zendesk_to_s3(ts, ds):
    ...
    tickets_response = get_tickets_updated_last_4_hours(ts_from, ts_until, 'salute', 1, url, API_KEY)
    ...
    filename = f"zendesk_salute/{exec_date}/tickets_zendesk_salute{date_aux}.csv"
    buffer.seek(0)
...
def _save_tickets_zendesk_in_postgres(ti):
    ...
    incremental_query = """
        INSERT INTO analytics_and_growth.tickets_zendesk_salute (id_ticket,"""+columns_query+""") 
        VALUES ("""+values_query+""")
...
with DAG(
    'etl_tickets_zendesk_salute_incremental_load_from_api',
