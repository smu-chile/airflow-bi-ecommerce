...
def _create_initial_order_item_promotions_table(ti):
    import numpy as np
    import pandas as pd
    import sqlalchemy
    ...
    truncate_query = "TRUNCATE TABLE ecommdata.orden_producto_promociones"
    connection.execute(text(truncate_query))
    connection.close()

    # Save to PostgreSQL:
    df.to_sql(name="orden_producto_promociones",
                con=engine,         
                schema="ecommdata",         
                if_exists='append',         
                index=False,         
    ...
    print("Data saved to PostgreSQL. Table: ecommdata.orden_producto_promociones")
...
with DAG(
    'etl_orden_producto_promociones_initial_load',
    default_args=default_args,
    description="Extracción y carga inicial de tabla orden_producto_promociones desde Janis Replica hasta Workspace.",
    schedule=None,
    ...
    t0 = PythonOperator(
        ...
        op_kwargs = {
            "query": """
                ...
                WHERE wo.id >= 168886
            """,
            "query_name": "wms_order_item_promotions",
        }
    )

    t1 = PythonOperator(
        task_id = "create_initial_orden_producto_promociones_table",
        python_callable = _create_initial_order_item_promotions_table
    )
