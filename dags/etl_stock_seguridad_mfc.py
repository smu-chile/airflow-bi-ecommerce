...
def ubicaciones_flo(ds):
    stock_tiendas_query = """select sap_code||'-'|| measurement_unit as ref_id,
                        '1917' as id_tienda,
                        date_part('dow','"""+ds+"""'::date+1) as dia,
                        date_part('week','"""+ds+"""'::date+1) as semana,
                        mfc_is_item_side
...
def stock_ventas_tienda_1917_to_s3(ds):
    import pandas as pd
    import numpy as np
    import io
    ...
    filename = f"stock_seguridad_mfc/{exec_date}/stock_seguridad_mfc_{date_aux}.csv"
    buffer.seek(0)
...
def carga_stock_seguridad_1917_janis(ds,ti):
    import requests
    import pandas as pd
    import datetime
    exec_date = ds.replace("-", "/")
    ...
    filename = ti.xcom_pull(key="return_value", task_ids=["stock_ventas_tienda_1917_to_s3"])[0]
    ...
    for i in range(len(df.index)):
        ...
        row = {"IdSku": material, "Quantity": 0, "Store": store, "MinStock": stock_seguridad, "Type": 2}
        print(row)
        payload.append(row)    
        if i % 499 == 0:
...
with DAG(
    ...
    catchup=False,
    tags=["DATA", "Janis", "ecommdata_unimarc", "stock", "stock_seguidad", "ventas", "unimarc","MFC"],
) as dag:
    ...
    t0 = PythonOperator(
        task_id = "stock_ventas_tienda_1917_to_s3",
        python_callable = stock_ventas_tienda_1917_to_s3,
    )

    t1 = PythonOperator(
        task_id = "carga_stock_seguridad_1917_janis",
