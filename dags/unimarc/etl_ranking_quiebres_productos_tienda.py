...
def load_ranking_quiebres_productos_tienda_to_postgres(ds):
    ...
    with open(curr_working_directory+f"/dags/unimarc/sql/ranking_quiebres_productos_tienda.sql", "r") as query_file:
...
with DAG(
    'etl_ranking_quiebres_productos_tienda',
    default_args=default_args,
    description="Extracción de datos de tabla ordenes janis  y posterior carga de ranking quiebres de SKUs -  tienda de ultimos 30 dias segmentados por tienda",
    schedule="0 7 * * *",
    start_date=pendulum.datetime(2022, 8, 11, tz="America/Santiago"),
    catchup=False,
    tags=["DATA", "ecommdata", "stock", "Unimarc", "Ordenes janis", "PATRICIO"],
    on_success_callback=dag_success_slack,
    on_failure_callback=dag_failure_slack,
) as dag:

    dag.doc_md = """
    Extracción de datos de tabla ordenes janis y posterior carga de quiebres de top 100 SKUs segmentados por tienda\n
    """ 
    ...
    t0 = PostgresOperator(
        ...
        sql="""
        truncate ecommdata.ranking_quiebres_productos_tienda
        """,
    )
