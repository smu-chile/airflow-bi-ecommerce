...
def ejecucion_principal(ds):
    ...
    # detalle ordenes
    #df_venta_fact_geo_tarifa_km.to_csv("archivos/output/costos_logisticos_diarios_estimacion_detalle_ordenes2.csv", index=False, decimal=',', sep=';')
    # tabla con ruta diferenciada
    #df_costo_transportadora_diario.to_csv("archivos/output/costos_logisticos_diarios_estimacion_transportadoras2.csv", index=False, decimal=',', sep=';')
...
def costos_to_sql(costos_df):
    ...
    #delete_query = "DELETE FROM forecast_and_planning.costos_logisticos_diarios"
...
def subir_a_bdd():
    ...
    #df_costos_real = pd.read_csv("archivos/output/costos_logisticos_diarios_real.csv", decimal=',', sep=';')
    ...
    #df_costos_real['id_tienda'] = df_costos_real['id_tienda'].apply(lambda x: str(x)[0:4].rstrip(' ').zfill(4))
    #df_costos = df_costos_estimado.merge(df_costos_real, on=['id_tienda', 'fecha'], how='outer').drop_duplicates(subset=['id_tienda', 'fecha'])
    #### nombres de columna
