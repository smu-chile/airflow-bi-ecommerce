WITH venta_neta AS (
    SELECT oj.fecha_facturacion AS fecha,
        TRUNC(
            SUM(
                CASE
        ...
        ) AS venta_total_neta_sin_descuentos,
        TRUNC(
            SUM(
                CASE
                    WHEN opp.id_orden_producto IS NOT NULL THEN opp.precio
    ...
    WHERE op.unidades_pickeadas IS NOT NULL
        AND oj.fecha_facturacion >= '{ds}'::date - interval '180 day'
    GROUP BY oj.fecha_facturacion
),
sustituciones AS (
    SELECT oj.fecha_facturacion AS fecha,
        (
            SUM(
                (
                    substitute.precio_lista * substitute.unidades_pickeadas
                ) - (
                    original.precio_lista * original.unidades_solicitadas
                )
            ) * -1
        ) AS perdida_sustitucion
    FROM ecommdata.orden_productos original
        JOIN ecommdata.orden_productos substitute ON substitute.id_producto_substituido = original.id
        LEFT JOIN ecommdata.productos p ON p.ref_id = original.ref_id
        LEFT JOIN ecommdata.marcas m ON p.id_marca = m.id
        LEFT JOIN ecommdata.categorias c ON p.id_categoria = c.id
        LEFT JOIN ecommdata.ordenes_janis oj ON oj.id = original.id_orden
    WHERE original.unidades_solicitadas > 0
        AND (
            substitute.precio_lista * substitute.unidades_pickeadas
        ) > (
            original.precio_lista * original.unidades_solicitadas
    ...
    SUM(
        CASE
            WHEN opp.ref_id IN (
                ...
                '882cc392-862e-4e9c-bdd8-41eb2f257fc4'
            ) THEN opp.valor
            ELSE 0
        END
    )::int AS descuento_cupones_crm,
    SUM(
        CASE
            WHEN (
                oj.fecha_facturacion::date > wp.fecha_fin_de_promocion::date
...
FROM ecommdata.ordenes_janis oj
    ...
    and oppe.campo = 'WORKFLOWID'
    LEFT JOIN ecommdata.productos p ON p.ref_id = op.ref_id
    LEFT JOIN sustituciones ON sustituciones.fecha = oj.fecha_facturacion
    LEFT JOIN venta_neta ON venta_neta.fecha = oj.fecha_facturacion
    left join ecommdata.workflow_promociones wp on op.ref_id = (
    ...
    and oppe.valor = wp.n_promocion::text
WHERE opp.nombre not ilike '%despacho%'
    AND oj.fecha_facturacion >= '{ds}'::date - interval '180 day'
GROUP BY oj.fecha_facturacion,
    sustituciones.perdida_sustitucion,
    venta_neta.venta_total_neta_sin_descuentos,
    venta_neta.venta_total_neta
ORDER BY oj.fecha_facturacion DESC;