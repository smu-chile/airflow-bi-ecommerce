...
def upload_refid_category(ti):
    ...
    sustituto_sin_update = ti.xcom_pull(task_ids="set_by_api_att_category")[0] # [ref_id]
    if  len(sustituto_sin_update) > 0:
    ...
    elif len(sustituto_sin_update) == 0:
        json_categories = ti.xcom_pull(task_ids="get_sustitutos_and_not_sustitutos")[0]
        df = pd.read_json(json_categories, orient='index')
    ...
    values ( %s, %s, %s) """
    print("Estableciendo conexión postgresql_conn")
    pg_hook = PostgresHook(postgres_conn_id="postgresql_conn")
    pg_connection = pg_hook.get_conn()
    cursor = pg_connection.cursor()
    print("Ejecutando TRUNCATE catalogo.sustitutos")
    cursor.execute("TRUNCATE catalogo.sustitutos")
    print("Ejecutando INSERT")
    cursor.execute(query_insert, lista_de_tuplas)
