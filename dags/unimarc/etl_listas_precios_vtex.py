...
def upload_fixed_prices(ti):
    ...
    df_data.to_sql(name="wp_listas_precios_vtex",
                   con=engine,
                   schema="catalogo",
                   if_exists='append',
                   index=False,
...
with DAG(
    ...
    description="Extracción y carga de la tabla wp_listas_precios_vtex desde API.",
    schedule_interval="0 22 * * *",
    start_date=pendulum.datetime(2023, 6, 5, tz="America/Santiago"),
    catchup=False,
    max_active_runs=1,
    tags=["vtex", "promociones", "listas_precios", "workflow_promociones", "precios"],
) as dag:

    dag.doc_md = """
    Extracción y carga de la tabla wp_listas_precios_vtex desde API.
    """
    ...
    t1 = PostgresOperator(
        ...
        sql="""
        TRUNCATE catalogo.wp_listas_precios_vtex
        """,
    )
