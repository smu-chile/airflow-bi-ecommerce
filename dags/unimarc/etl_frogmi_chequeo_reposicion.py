...
def _load_dw_stock_to_s3(ds,ts):
    ...
    dw_conn = jaydebeapi.connect(jdbc_driver_name, 
    ...
    prefix = "frogmi/check_reposicion_sap/"+curr_datetime
    file_name = prefix+"check_reposicion_sap.csv"
...
def _get_table_chequeo_reposicion_from_S3(ti):
    import pandas as pd

    chequeo_reposicion_file = ti.xcom_pull(key="return_value", task_ids=["load_dw_stock_to_s3"])[0]
    s3_bucket = Variable.get("AWS_S3_BUCKET_NAME")
    s3_hook = S3Hook(aws_conn_id="aws_s3_connection")

    print("Searching file: "+chequeo_reposicion_file)
    if not s3_hook.check_for_key(chequeo_reposicion_file, bucket_name=s3_bucket):
        raise Exception("Key %s does not exist." % chequeo_reposicion_file)

    chequeo_reposicion_object = s3_hook.get_key(chequeo_reposicion_file, bucket_name=s3_bucket)
...
def _save_table_chequeo_reposicion(ts, ti, ds):
    import pandas as pd
    import sqlalchemy

    df = _get_table_chequeo_reposicion_from_S3(ti)
    ...
    with engine.begin() as conn:
        df.to_sql(name="frogmi_chequeo_reposicion",
                con=engine,         
                schema="ecommdata",         
                if_exists='append',         
                index=False,         
...
with DAG(
    ...
    start_date=pendulum.datetime(2022, 10, 26, tz="America/Santiago"),
    catchup=False,
    max_active_runs = 1,
    ...
    t1 = PythonOperator(
        task_id = "save_table_chequeo_reposicion",
        python_callable = _save_table_chequeo_reposicion
    )
