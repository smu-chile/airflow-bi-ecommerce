...
def _load_ticket_zendesk_to_s3(ts, ds):
    ...
    tickets_response = get_tickets_updated_last_4_hours(ts_from, ts_until, 'unimarc', 1, url, API_KEY)
    ...
    for ticket in tickets:
        ...
        ticket_json = {
            ...
            'id_reclamo_sernac': get_value_from_id_field_numeric(5262834532375, array_campos_personalizados),
    ...
    filename = f"zendesk/{exec_date}/tickets_zendesk{date_aux}.csv"
    buffer.seek(0)
...
def _save_tickets_zendesk_in_postgres(ti):
    ...
    df['id_tienda'] = df['id_tienda'].apply(lambda x: str(int(x)).zfill(4) if pd.notna(x) and x == x else np.nan)
