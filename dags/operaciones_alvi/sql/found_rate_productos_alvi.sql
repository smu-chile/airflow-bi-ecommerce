insert into operaciones_alvi.found_rate_productos
select 
orden
, fecha_facturacion 
, id_producto_orden
...
select 	distinct oj.id			as orden
    ...
    inner join ecommdata_alvi.orden_productos op on oj.id = op.id_orden
    left join ecommdata_alvi.orden_productos op2 on op2.id = op.id_producto_substituido
    left join ecommdata_alvi.orden_productos op3 on op.id = op3.id_producto_substituido
	left join 	(
		        SELECT opp.id_orden
		        , opp.id_orden_producto
		        , round(sum(opp.peso)) AS peso_pickeado
		        FROM ecommdata_alvi.orden_producto_pesables opp 
	...
	and oj.janis_id in {{ti.xcom_pull(key="return_value", task_ids=['get_query_order_ids_from_s3'])[0]}}
	  ) a
