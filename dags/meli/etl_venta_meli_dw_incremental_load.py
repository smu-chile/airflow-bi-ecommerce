...
def _ventas_dw_meli_incremental_load(ti):
    import pandas as pd
    import numpy as np
    ...
    incremental_query = """
        INSERT INTO ecommdata_meli.ventas_meli_datawarehouse (id,"""+columns_query+""") 
        VALUES ("""+values_query+""")
        ON CONFLICT (id)
...
with DAG(
    ...
    max_active_runs=1,
    concurrency=2,
    tags=["DATA", "DW", "S3", "workspace", "ventas_meli_datawarehouse", "mercado_libre", "SERGIO"],
) as dag:
    ...
    t1 = PythonOperator(
        task_id = "_ventas_dw_meli_incremental_load",
        python_callable = _ventas_dw_meli_incremental_load,
        pool = "backfill_pool"
    )
