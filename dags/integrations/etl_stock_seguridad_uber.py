...
def stock_ventas_tiendas_to_s3_am(ds):
    ...
    filename = f"stock_seguridad_uber/{exec_date}/stock_seguridad_uber_am_{date_aux}.csv"
    buffer.seek(0)
...
def stock_ventas_tiendas_to_s3_pm(ds):
    ...
    buffer = io.StringIO()
    df_final.to_csv(buffer, header=True, index=False, encoding="utf-8")
    filename = f"stock_seguridad_uber/{exec_date}/stock_seguridad_uber_pm_{date_aux}.csv"
    buffer.seek(0)
    print("se logro transformar el dataframe a un archivo .csv")
    print(f"con fecha {ds} y nombre de filename como {filename}")
    s3_hook.load_string(buffer.getvalue(),
...
def stock_seguridad_to_postgres_am(ti):
    ...
    with engine.begin() as conn:
        conn.execute("truncate table integraciones.stock_seguridad_uber")
        df.to_sql(name="stock_seguridad_uber",
                    con=conn,         
...
with DAG(
    ...
    dag.doc_md = """
    Carga stock de seguridad de uber\n
    guardar en S3.
    """ 
