...
def _join_stock_and_promo_prices_from_s3(ds, ti,ts):
    ...
    for store_id in rappi_store_ids:
        ...
        join_file_name = f"integraciones/last_millers/stock/out/rappi/{exec_date}/{aux_date}/{store_id}.json"
...
def _send_joined_data_to_api(ds,ts):
    ...
    prefix = f"integraciones/last_millers/stock/out/rappi/{exec_date}/{aux_date}/"
    ...
    responses_prefix = f"rappi/api/stock/post/full/responses/{exec_date}/{aux_date}/"
...
with DAG(
    ...
    start_date=pendulum.datetime(2024, 6, 12, tz="America/Santiago"),
    catchup=False,
    max_active_runs=1,
    concurrency=2,
    tags=["OPS", "last_millers", "dw", "stock", "precios", "NICOLAS","Rappi"],
) as dag:
